  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-gray-400 mt-1">Deep dive into your campaign performance data.</p>
      </div>

      {/* Top Metrics */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { label: 'Total Impressions', value: '109.8M', change: '+22%', icon: Eye, color: 'text-blue-400 bg-blue-500/10 border-blue-500/20' },
          ...
          { label: 'Avg. ROAS', value: '4.72x', change: '+0.8x', icon: TrendingUp, color: 'text-amber-400 bg-amber-500/10 border-amber-500/20' },
        ...
        >
          ...
          <ResponsiveContainer width="100%" height={260}>
            <BarChart data={channelBreakdown} layout="vertical">
              <CartesianGrid strokeDasharray="3 3" stroke="#ffffff08" />
              <XAxis type="number" stroke="#6b7280" tick={{ fill: '#6b7280', fontSize: 11 }} tickFormatter={(v: number) => `$${(v / 1000).toFixed(0)}k`} />
              <YAxis type="category" dataKey="channel" stroke="#6b7280" tick={{ fill: '#6b7280', fontSize: 11 }} width={80} />
              <Tooltip contentStyle={{ backgroundColor: '#1e293b', border: '1px solid rgba(255,255,255,0.1)', borderRadius: '12px', color: '#f8fafc' }} />
              <Legend wrapperStyle={{ color: '#9ca3af', fontSize: '12px' }} />
